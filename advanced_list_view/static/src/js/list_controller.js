...
/**
 * Handle the click event for generating and downloading PDF file.
 */
ListController.prototype._onClickPDF = async function() {
    var self = this;
    ...
    ajax.jsonRpc('/get_data', 'call', {
        ...
        return self.model.action.doAction({
            ...
            report_name: 'advanced_list_view.print_pdf_listview',
            report_file: "advanced_list_view.print_pdf_listview",
            data: {
                'length': length_field,
                'record': data
            }
...
/**
 * Handle the click event for generating and downloading an Excel file.
 */
ListController.prototype._onClickExcel = async function() {
    // Retrieve the fields to export
    const fields = this.props.archInfo.columns
        .filter((col) => col.optional === false || col.optional === "show")
        .map((col) => this.props.fields[col.name])
...
/**
 * Handle the click event for generating and downloading a CSV file.
 */
ListController.prototype._onClickCSV = async function() {
    // Retrieve the fields to export
    const fields = this.props.archInfo.columns
        .filter((col) => col.optional === false || col.optional === "show")
        .map((col) => this.props.fields[col.name])
...
/**
 * Handle the click event for copying the records into clipboard.
 */
ListController.prototype._onClickCopy = async function() {
    var self = this;
    // Retrieve the fields to export
    const fields = this.props.archInfo.columns
