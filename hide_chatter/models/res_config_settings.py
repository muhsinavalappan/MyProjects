...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        res.update(model_ids=[
            (6, 0, literal_eval(selected_models))]
        if selected_models else False, )
        # for i in literal_eval(selected_models):
        #     result = self.env['ir.model'].search([('id', '=', i)])
        return res
